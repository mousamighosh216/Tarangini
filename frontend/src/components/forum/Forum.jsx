...
const AVAILABLE_TAGS = ["general","wellness","pcos","cycle","nutrition","lifestyle","question","discussion","help","showcase"];
const NAV = [
...
const TAG_COLORS = {
  pcos: ["#EDE9FA","#C5BDED","#7B6EC4"],
  wellness: ["#E6F5F0","#A8D8C8","#4A9E85"],
  cycle: ["#FEF6DC","#F5D97A","#B89020"],
  general: ["#F0EEFF","#C8B8F8","#6040C0"],
  discussion: ["#FAE8E6","#F2C4BF","#D4736A"],
  nutrition: ["#E6F0FF","#A8C8F0","#4A78C0"],
  lifestyle: ["#FFF3E6","#F5C880","#B07820"],
  question: ["#E6FFF0","#90D8A8","#3A8060"],
...
const SAMPLE_POSTS = [
  {
    id:"p1", title:"My PCOS journey — 6 months update",
    content:"After 6 months of tracking consistently, I've noticed some patterns I never saw before. My cycles are becoming more regular and the Tarangini predictions have been spot on. Sharing for anyone else who is just starting out.",
    author:"priya_w", timestamp:Date.now()-1000*60*60*14,
    tags:["pcos","wellness"], votes:18,
    comments:[
      {id:"c1",author:"meera_h",content:"Thank you for sharing! I'm only 2 months in but already seeing differences too. The ovulation tracking especially has been helpful.",timestamp:Date.now()-1000*60*60*12,votes:6,replies:[
        {id:"r1",author:"priya_w",content:"Keep going! The first 3 months feel slow but then it clicks.",timestamp:Date.now()-1000*60*60*11,votes:3}
      ]},
      {id:"c2",author:"aarti_d",content:"Did you make any diet changes alongside? I've been focusing on low GI foods and it's helped my energy levels a lot.",timestamp:Date.now()-1000*60*60*9,votes:4,replies:[]}
    ]
  },
  {
    id:"p2", title:"Best nutrition tips for cycle phase 2?",
    content:"I'm in the follicular phase and curious what foods others focus on during this time. I've read about estrogen-supporting foods but would love real experience from this community.",
    author:"sara_k", timestamp:Date.now()-1000*60*60*38,
    tags:["nutrition","cycle","question"], votes:24,
    comments:[
      {id:"c3",author:"nadia_r",content:"Leafy greens and flaxseeds have been game changers for me during this phase! Also fermented foods.",timestamp:Date.now()-1000*60*60*35,votes:9,replies:[
        {id:"r2",author:"sara_k",content:"Adding flaxseeds today! Thanks so much.",timestamp:Date.now()-1000*60*60*34,votes:5}
      ]}
    ]
  },
  {
    id:"p3", title:"30-day yoga challenge update 🧘",
    content:"I started a daily yoga practice last month focusing on poses that support hormonal balance. My PMS symptoms have noticeably reduced. Sharing the sequence I've been following if anyone wants it!",
    author:"deepa_m", timestamp:Date.now()-1000*60*47,
    tags:["lifestyle","wellness"], votes:11, comments:[]
  }
];
...
function VoteBtn({ count, voted, onUp, onDown, vertical=false }){
  ...
  return (
    <div style={{ display:"flex", flexDirection:vertical?"column":"row", alignItems:"center", gap:2 }}>
      ...
      <span style={{ fontSize:13, fontWeight:800, color:voted!==0?C.primary:C.muted, minWidth:20, textAlign:"center" }}>{count}</span>
...
function Comment({ comment, username, level=1, onReply, onVote }){
  ...
  const isL2 = level === 2;
  return (
    <div style={{ marginLeft:isL2?36:0, borderLeft:isL2?`2px solid ${C.primaryL}`:"none", paddingLeft:isL2?16:0 }}>
      <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <p style={{ margin:"0 0 8px", fontSize:13.5, lineHeight:1.7, color:"#3D2C30", whiteSpace:"pre-wrap" }}>{comment.content}</p>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <VoteBtn count={comment.votes} voted={comment._voted||0} onUp={() => onVote(comment.id,1)} onDown={() => onVote(comment.id,-1)} />
            {level===1 && (
              <button onClick={() => setReplying(r=>!r)} style={{
                background:replying?C.primaryBg:"transparent",
                border:`1px solid ${replying?C.primaryL:C.border}`,
                borderRadius:20, fontFamily:"inherit", fontSize:11.5, fontWeight:600,
                color:replying?C.primary:C.muted, padding:"3px 12px", cursor:"pointer", transition:"all 0.15s" }}>
                {replying ? "Cancel" : "Reply"}
              </button>
            )}
          </div>
          {replying && (
            <ReplyBox onSubmit={t=>{onReply(comment.id,t);setReplying(false);}} onCancel={()=>setReplying(false)} />
          )}
          {level===1 && comment.replies?.length>0 && (
            <div style={{ marginTop:14, display:"flex", flexDirection:"column", gap:12 }}>
              {comment.replies.map(r => (
                <Comment key={r.id} comment={r} username={username} level={2} onReply={()=>{}} onVote={onVote} />
              ))}
            </div>
          )}
        </div>
  ...
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ background:C.card, borderRadius:18, border:`1.5px solid ${hov?C.primaryL:C.border}`,
        padding:"18px 20px", cursor:"pointer", transition:"all 0.18s", boxShadow:hov?C.shadowMd:C.shadow }}>
      <div style={{ display:"flex", gap:14, alignItems:"flex-start" }}>
        <div onClick={e=>e.stopPropagation()} style={{ paddingTop:4 }}>
          <VoteBtn count={post.votes} voted={post._voted||0} vertical onUp={()=>onVote(post.id,1)} onDown={()=>onVote(post.id,-1)} />
        </div>
        <div style={{ flex:1, minWidth:0 }} onClick={()=>onOpen(post)}>
          <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:9 }}>
            {post.tags.map(t => <span key={t} style={tagStyle(t,false)}>{t}</span>)}
          </div>
          <h2 style={{ margin:"0 0 7px", fontSize:15.5, fontWeight:800, lineHeight:1.35, color:C.text, letterSpacing:"-0.25px" }}>{post.title}</h2>
...
function PostView({ post, username, onBack, onUpdate }){
  ...
  return (
    <div>
      <button onClick={onBack} style={{ background:"none", border:"none", cursor:"pointer", fontFamily:"inherit",
      ...
      <div style={{ background:C.card, borderRadius:20, border:`1.5px solid ${C.border}`,
        ...
        <div style={{ display:"flex", gap:16, alignItems:"flex-start" }}>
          <VoteBtn count={post.votes} voted={post._voted||0} vertical onUp={()=>votePost(1)} onDown={()=>votePost(-1)} />
          <div style={{ flex:1 }}>
            <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:12 }}>
              {post.tags.map(t => <span key={t} style={tagStyle(t,false)}>{t}</span>)}
            </div>
            <h1 style={{ margin:"0 0 14px", fontSize:22, fontWeight:800, lineHeight:1.3, color:C.text, letterSpacing:"-0.5px" }}>{post.title}</h1>
            <p style={{ margin:"0 0 18px", fontSize:14.5, lineHeight:1.8, color:"#3D2C30", whiteSpace:"pre-wrap" }}>{post.content}</p>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        <span style={{ fontWeight:800, fontSize:14, color:C.text }}>💬 {cc} Comment{cc!==1?"s":""}</span>
        <button onClick={() => setShowReply(r=>!r)} style={{ fontFamily:"inherit", fontSize:13, fontWeight:700,
          background:showReply?"transparent":`linear-gradient(135deg,${C.primary},#C05A80)`,
          color:showReply?C.muted:"#fff", border:`1.5px solid ${showReply?C.border:C.primary}`,
      ...
      {showReply && (
        <div style={{ background:C.card, borderRadius:16, border:`1.5px solid ${C.border}`,
          ...
          <ReplyBox placeholder="Share your thoughts…" onSubmit={t=>{addComment(t);setShowReply(false);}} onCancel={()=>setShowReply(false)} />
        </div>
      )}
      {post.comments.length===0 ? (
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {post.comments.map(c => (
            <div key={c.id} style={{ background:C.card, borderRadius:16, border:`1.5px solid ${C.border}`, padding:"16px 18px", boxShadow:C.shadow }}>
              <Comment comment={c} username={username} level={1} onReply={addReply} onVote={voteComment} />
            </div>
          ))}
        </div>
      )}
...
function CreatePostModal({ username, onClose, onCreate }){
  ...
  return (
    <div onClick={e=>{if(e.target===e.currentTarget)onClose();}}
      style={{ position:"fixed", inset:0, background:"rgba(44,32,37,0.35)", backdropFilter:"blur(4px)",
        display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:24 }}>
      <div style={{ background:C.card, borderRadius:26, width:"100%", maxWidth:540,
        boxShadow:"0 24px 70px rgba(212,115,106,0.22)", border:`1.5px solid ${C.border}`, overflow:"hidden" }}>
        <div style={{ background:`linear-gradient(135deg,${C.primaryBg},${C.lavBg})`,
          ...
          <span style={{ fontWeight:800, fontSize:16, color:C.primary, letterSpacing:"-0.3px" }}>✦ New Post</span>
          <button onClick={onClose} style={{ background:"rgba(212,115,106,0.15)", border:"none", borderRadius:"50%",
            width:30, height:30, cursor:"pointer", fontSize:18, color:C.primary,
            display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700 }}>×</button>
        </div>
        <div style={{ padding:"22px 26px", display:"flex", flexDirection:"column", gap:16 }}>
          <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Post title…"
            style={{ ...inputStyle, fontSize:16, fontWeight:800, letterSpacing:"-0.25px" }} />
          <textarea value={content} onChange={e=>setContent(e.target.value)}
            placeholder="What's on your mind?" rows={5} style={{ ...inputStyle, resize:"vertical" }} />
          <div>
          ...
          <button onClick={() => { if(valid) onCreate({ id:uid(), title:title.trim(), content:content.trim(),
            author:username, timestamp:Date.now(), tags:selectedTags, votes:0, _voted:0, comments:[] }); }}
            disabled={!valid}
            style={{ fontFamily:"inherit", fontSize:14, fontWeight:800,
              ...
              padding:"13px 0", cursor:valid?"pointer":"not-allowed", transition:"all 0.15s" }}>
            Publish Post ✦
          </button>
        </div>
      </div>
...
/* ── Root ── */
export default function Forum({ userName = "user" }){
  ...
  // Try to load posts from storage
  useEffect(() => {
    (async () => {
      try { const r = await window.storage.get("lp1"); if(r) setPosts(JSON.parse(r.value)); } catch{}
    })();
  }, []);

  const savePosts = async next => {
    setPosts(next);
    try { await window.storage.set("lp1", JSON.stringify(next)); } catch{}
  };
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans',sans-serif", color:C.text, position:"relative" }}>
      <style>{`
      ...
      {isMobile && mobileSidebarOpen && (
        <div onClick={() => setMobileSidebarOpen(false)}
          style={{ position:"fixed", inset:0, background:"rgba(44,32,37,0.35)", zIndex:150, backdropFilter:"blur(2px)" }} />
      )}

      {/* ─── Forum Sidebar ─── */}
      <aside style={{
        width: 220, background:C.sidebar, borderRight:`1.5px solid ${C.border}`,
        display:"flex", flexDirection:"column", padding:"0 0 20px",
        position: isMobile ? "fixed" : "sticky",
        left: isMobile ? (mobileSidebarOpen ? 0 : "-240px") : 0,
        top: 0, height:"100vh", flexShrink:0, zIndex:160,
        ...
        <div style={{ padding:"22px 20px 18px", borderBottom:`1.5px solid ${C.border}` }}>
          {isMobile && (
            <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:"8px" }}>
              <button onClick={() => setMobileSidebarOpen(false)}
                style={{ background:"none", border:"none", cursor:"pointer", fontSize:"20px", color:"#bbb", padding:"2px 6px" }}>×</button>
            </div>
          )}
          <div style={{ display:"flex", alignItems:"center", gap:11 }}>
            ...
            <div>
              <div style={{ fontFamily:"'Playfair Display',serif", fontWeight:700, fontSize:17, color:C.text, lineHeight:1 }}>Lounge</div>
              <div style={{ fontSize:10, color:C.muted, letterSpacing:"0.04em", marginTop:3 }}>Community Space</div>
            </div>
          </div>
        </div>
        {/* nav */}
        <nav style={{ padding:"16px 12px", flex:1 }}>
          {NAV.map(n => {
            ...
            return (
              <button key={n.id} onClick={() => { setNavActive(n.id); setView("home"); setActivePostId(null); if(n.id!=="tags") setFilterTag(null); if(isMobile) setMobileSidebarOpen(false); }}
                style={{ display:"flex", alignItems:"center", gap:12, width:"100%", padding:"10px 14px",
                  borderRadius:12, border:"none", cursor:"pointer", fontSize:13.5, fontWeight:active?700:500,
                  textAlign:"left", marginBottom:4, background:active?C.primaryBg:"transparent",
        ...
        <div style={{ padding:"0 12px 14px" }}>
          <button onClick={() => { setShowCreate(true); if(isMobile) setMobileSidebarOpen(false); }}
        ...
        {/* user pill */}
        <div style={{ padding:"12px 16px 0", borderTop:`1.5px solid ${C.border}`, display:"flex", alignItems:"center", gap:10 }}>
          <Avatar name={username || "U"} size={34} />
          <div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
        {/* header */}
        <header style={{
          background:`linear-gradient(135deg,${C.primary} 0%,#C05A80 100%)`,
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap:12, flex:1, minWidth:0 }}>
              ...
              {isMobile && (
                <button onClick={() => setMobileSidebarOpen(true)}
                  style={{ background:"rgba(255,255,255,0.2)", border:"none", cursor:"pointer",
                    borderRadius:"8px", padding:"8px", flexShrink:0, display:"flex", flexDirection:"column",
              ...
              <div style={{ minWidth:0 }}>
                <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize: isMobile ? "20px" : "27px", fontWeight:700,
                ...
                <p style={{ color:"rgba(255,255,255,0.78)", fontSize: isMobile ? "12px" : "13px", margin:0 }}>
            ...
            {view==="home" && (
              <div style={{ background:"rgba(255,255,255,0.18)", borderRadius:12,
                ...
                fontSize: isMobile ? "11px" : "12.5px", color:"rgba(255,255,255,0.92)", fontWeight:700,
                backdropFilter:"blur(8px)", border:"1px solid rgba(255,255,255,0.28)", flexShrink:0 }}>
        ...
        {/* content */}
        <main style={{ flex:1, padding: isMobile ? "16px" : "28px 32px", maxWidth:820, width:"100%" }}>
          {view==="home" && (
            <>
              {allTags.length > 0 && (
                <div style={{ background:C.card, borderRadius:16, border:`1.5px solid ${C.border}`,
                  padding:"16px 20px", marginBottom:22, boxShadow:C.shadow }}>
                  <p style={{ fontSize:11.5, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.07em", color:C.muted, margin:"0 0 10px" }}>Filter by Tag</p>
                  <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                    <span style={tagStyle("all", !filterTag)} onClick={() => setFilterTag(null)}>✦ All</span>
                    {allTags.map(t => (
                      <span key={t} style={tagStyle(t, filterTag===t)} onClick={() => setFilterTag(t===filterTag ? null : t)}>{t}</span>
                    ))}
                  </div>
                </div>
              )}
